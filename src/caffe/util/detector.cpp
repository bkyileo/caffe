...
void Detector::Set_Model(std::string &proto_file, std::string &model_file) {
  ...
  const std::string roi_name = "psroipooled_cls_rois";
  for (size_t i = 0; i < layer_names.size(); i++) {
    if (roi_name.size() > layer_names[i].size()) continue;
    if (roi_name == layer_names[i].substr(0, roi_name.size())) {
      CHECK_EQ(this->roi_pool_layer, -1) << "Previous psroipooled_cls_rois layer : " << this->roi_pool_layer << " : " << layer_names[this->roi_pool_layer];
      this->roi_pool_layer = i;
    }
  }
  CHECK(this->roi_pool_layer >= 0 && this->roi_pool_layer < layer_names.size());
  DLOG(INFO) << "SET MODEL DONE, psroipooled_cls_rois LAYER : " << layer_names[this->roi_pool_layer];
...
void Detector::predict(const cv::Mat &img_in, std::vector<caffe::BBox<float> > &results) {
...
void Detector::predict_original(const cv::Mat &img_in, std::vector<caffe::BBox<float> > &results) {

  CHECK(FrcnnParam::test_scales.size() == 1) << "Only single-image batch implemented";

  float scale_factor = caffe::get_scale_factor(img_in.cols, img_in.rows, FrcnnParam::test_scales[0], FrcnnParam::test_max_size);
  ...
  CHECK_EQ(cls_num , caffe::FrcnnParam::n_classes);
  results.clear();
  ...
  for (int cls = 1; cls < cls_num; cls++) { 
    ...
    for (int i = 0; i < box_num; i++) { 
      ...
      Point4f<float> box = caffe::bbox_transform_inv(roi, delta);
...
void Detector::predict_iterative(const cv::Mat &img_in, std::vector<caffe::BBox<float> > &results) {
