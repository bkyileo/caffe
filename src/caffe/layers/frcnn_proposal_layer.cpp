...
#ifndef CPU_ONLY
  CUDA_CHECK(cudaMalloc(&anchors_, sizeof(float) * anchorss.size()));
  CUDA_CHECK(cudaMemcpy(anchors_, &(anchorss[0]),
                        sizeof(float) * anchorss.size(), cudaMemcpyHostToDevice));

  const int rpn_pre_nms_top_n = 
    this->phase_ == TRAIN ? 12000 : 6000;
...
void FrcnnProposalLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
  ...
  const int config_n_anchors = anchorss.size() / 4;
    ...
    for (int i = 0; i < width; i++) {
      for (int k = 0; k < config_n_anchors; k++) {
        ...
        Point4f<Dtype> anchor(
            anchorss[k * 4 + 0] + i * 16,  // shift_x[i][j];
            anchorss[k * 4 + 1] + j * 16,  // shift_y[i][j];
            anchorss[k * 4 + 2] + i * 16,  // shift_x[i][j];
            anchorss[k * 4 + 3] + j * 16); // shift_y[i][j];
